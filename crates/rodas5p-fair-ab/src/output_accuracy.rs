...
pub enum AccuracyBudgetVerdict { WithinBudget, OutsideBudget, ReferenceUnresolved, BudgetNotSpecified }
...
pub fn assess_error_budget(error: f64, uncertainty: f64, budget: Option<f64>) -> FairResult<ErrorBudgetAssessment> {
    if ![error, uncertainty].iter().all(|v| v.is_finite() && *v >= 0.0)
        || budget.is_some_and(|v| !v.is_finite() || v < 0.0) {
        return Err(FairError::Invalid("finite nonnegative error, reference uncertainty and budget required".into()));
    }
    let lower_error_wrms = (error - uncertainty).max(0.0);
    let upper_error_wrms = error + uncertainty;
    if !upper_error_wrms.is_finite() { return Err(FairError::Invalid("unrepresentable upper error bound".into())); }
    ...
    Ok(ErrorBudgetAssessment { lower_error_wrms, upper_error_wrms, budget_wrms: budget, verdict })
}
/// Additive read-only assessment; the legacy direct-trajectory gate is retained.
pub fn assess_output_accuracy(evidence: &DualOutputPolicyEvidence, budget: Option<f64>) -> FairResult<OutputAccuracyAssessment> {
    let policy_sensitivity = evidence.classify()?;
    let u = evidence.reference_wrms_basis.error_scale.reference_uncertainty_wrms;
