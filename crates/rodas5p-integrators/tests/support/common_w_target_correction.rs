...
use rodas5p_core::{
    CoreError, CoreResult, LinearOperator, LuFactorization, WorkCounters, apply_jvp_counted,
};
use rodas5p_integrators::StepContext;
pub fn common_w_target_correction(
    ...
    if n == 0
        || states.len() != s
        || rhs.len() != s
        || states.iter().chain(rhs).any(|v| v.len() != n)
    {
    ...
    if !context.problem.supports_matrix_free_jvp() {
        return Err(CoreError::InvalidInput("analytic JVP required".into()));
    }
    let w = context
        .shifted
        .explicit()
        .ok_or_else(|| CoreError::InvalidInput("explicit common W required".into()))?;
    ...
    for i in 0..s {
        p.fill(0.0);
        q.fill(0.0);
        for (j, z) in solution.iter().enumerate() {
            for k in 0..n {
        ...
        if i > 0 {
            let ji = context
                .problem
                .linearize_matrix_free(context.t + context.coeffs.c[i] * context.h, &states[i])?;
        ...
        if corrected.iter().any(|v| !v.is_finite()) {
            return Err(CoreError::NonFinite("correction RHS".into()));
        }
        work.direct_solve_calls += 1;
        let row = factor.solve(&corrected)?;
        if row.iter().any(|v| !v.is_finite()) {
            return Err(CoreError::NonFinite("correction solution".into()));
        }
        solution.push(row);
    }
    Ok(solution)
