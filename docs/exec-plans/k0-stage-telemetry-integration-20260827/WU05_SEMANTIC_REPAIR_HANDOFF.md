...
Use the exact `K0_PACKAGE_SHA` delivered with this handoff. The runner accepts the existing c6-prepared merge as `UPGRADE`, validates the new package in a detached worktree, then creates an ordered `[f6208a10..., K0_PACKAGE_SHA]` merge. Exact successful retry is idempotent.
...
Require `mode: UPGRADE`, all existing package/supplement markers, and `LOCAL_WU05_AUTHORITY_READY`. If local full SHA/tree differ from the exact values above, stop as authority drift; do not guess or discard commits.
...
No external ZIP or host-side continuation bundle is required after this package is published. Extract the entry point directly from the exact package commit. A missing old local ZIP is packaging/transport absence, not a scientific blocker. The exact Git commit now materializes every required control byte.
